...
def prepare_data(session: Session) -> None:
    admin = UserCreate(full_name="Jon Doe", is_active=True, username="petrov", role="admin", password=app_config.ADMIN_PASSWORD)
    user = create_user(session=session, user_create=admin)
